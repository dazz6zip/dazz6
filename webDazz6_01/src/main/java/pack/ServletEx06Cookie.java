...
public class ServletEx06Cookie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String pwd = null;
		try {
			Cookie[] cookies = request.getCookies(); // 클라이언트의 모든 쿠키 읽기 (MAX 300)
		...
		out.println("<form method='post'>"); // submit 처리 -> doPost
...
//		System.out.println(id + " " + pwd);
		...
		if (id.equals("aaa") && pwd.equals("111")) {
			...
			try {
				...
				Cookie pwdCookie = new Cookie("pwd", pwd); // 암호화된 id로 쿠키 만들기
				pwdCookie.setMaxAge(60 * 60 * 24 * 365); // 쿠키 유효기간 
				
				response.addCookie(idCookie);
				response.addCookie(pwdCookie); // 클라이언트 컴퓨터에 쿠키 저장

				out.println("로그인 성공 (쿠키 설정 완료)");
			} catch (Exception e) {
				out.println("로그인 시도 실패");
			}
		} else {
			out.println("로그인 실패");
		}
